...
async def cmd_dig(message: types.Message, bot_state: BotState):
    ...
    # In-memory lock для предотвращения параллельной обработки
    dig_lock = get_dig_lock(user_id_str, bunker_id)

    # Если lock уже занят — запрос уже обрабатывается, игнорируем
    if dig_lock.locked():
        return

    async with dig_lock:
        # Проверяем подписку и cooldown параллельно
        ...
        try:
            ...
            if random.random() < 0.01 and digger_data.get("last_loot_type") != "super":
                ...
                if is_new_user:
                    ...
                    if is_success:
                        event = random.choice(
                            messages.get("success", [{"text": "Нашёл {} ГП-5!", "image": "success.jpg"}]))
                        loot = random.randint(1, 5)
                        event_text = event["text"].format(loot)
                        loot_type = "normal"
                    else:
...
async def cmd_box(message: types.Message, bot_state: BotState):
    ...
    async with box_lock:
        ...
        if not can_open:
            ...
            return  # Убедимся что return есть
...
async def cmd_check_user(message: types.Message, bot_state: BotState):
    """Проверка данных пользователя по всем чатам"""
    if not is_admin(message.from_user.id, bot_state):
        return
    ...
    # Получаем данные из global_stats
    global_doc = await db['global_stats'].find_one({'_id': target_user_id})
    stored_total = global_doc.get('total_gp5', 0) if global_doc else 0
    old_gp5 = global_doc.get('gp5', 'не было') if global_doc else 'не было'

    result = (
            f"📊 **Проверка пользователя** `{target_user_id}`\n\n"
            f"**По чатам:**\n" + "\n".join(chats_info) + "\n\n"
                                                         f"**Сумма по чатам:** {total_sum}\n"
                                                         f"**В global_stats.total_gp5:** {stored_total}\n"
                                                         f"**Старое поле gp5 (максимум):** {old_gp5}\n\n"
    )

    if total_sum == stored_total:
        result += "✅ Данные корректны!"
    else:
        result += f"⚠️ Расхождение: {stored_total - total_sum}"
